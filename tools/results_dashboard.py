...
class ResultIndex:
    """Read the stable, lightweight index of one visualization run."""

    def __init__(self, result_dir: Path, data_indices: list[int] | None = None):
        self.root = result_dir.expanduser().resolve()
        if not self.root.is_dir():
            raise ValueError(f"结果目录不存在或不是目录: {self.root}")
        samples_dir = self.root / "samples"
        if not samples_dir.is_dir():
            raise ValueError(f"结果目录缺少 samples/: {self.root}")
        ...
        self.step_names = self._discover_steps(run_data)

    @staticmethod
    def _select_sample_ids(ordered_ids: list[str], data_indices: list[int]) -> list[str]:
        if not data_indices:
            raise ValueError("数据索引列表不能为空")
        if len(data_indices) != len(set(data_indices)):
            raise ValueError("数据索引列表不能包含重复值")
        invalid = [index for index in data_indices if index < 0 or index >= len(ordered_ids)]
        if invalid:
            valid_range = f"0 到 {len(ordered_ids) - 1}" if ordered_ids else "空"
            raise ValueError(
                f"数据索引越界: {', '.join(map(str, invalid))}（有效范围: {valid_range}）"
            )
        return [ordered_ids[index] for index in data_indices]

    def _read_run_data(self) -> dict[str, object]:
    ...
    def _load_sample(self, samples_dir: Path, task_id: str) -> Sample:
        return Sample(task_id=task_id, path=samples_dir / task_id)

    def _discover_steps(self, run_data: dict[str, object]) -> list[str]:
        generation = run_data.get("generation", {})
        expected = generation.get("max_denoising_steps") if isinstance(generation, dict) else None
    ...
    def page(self, page: int, page_size: int) -> dict[str, object]:
        if page < 1:
        ...
        items = [self._serialize_sample(sample) for sample in self.samples[start : start + page_size]]
        return {
            "items": items,
            "page": page,
            "page_size": page_size,
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if parsed.path == "/api/manifest":
            self._send_json(
                {
                    "run_name": self.server.index.root.name,
    ...
    def _serve_samples(self, query: str) -> None:
        ...
        try:
            ...
            payload = self.server.index.page(page, page_size)
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .table-shell { margin:0 24px 26px; overflow:auto; border-radius:18px; border:1px solid var(--line); background:var(--card); box-shadow:0 18px 50px rgba(25,43,68,.14); max-height:calc(100vh - 210px); }
...
def main() -> None:
    ...
    print(f"[dashboard] run: {index.root.name}")
